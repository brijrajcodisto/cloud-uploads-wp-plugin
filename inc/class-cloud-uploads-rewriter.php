...
/**
 * Cloud_Uploads_Rewriter
 *
 * @since 1.0
 */

class Cloud_Uploads_Rewriter {
	...
	function __construct( $uploads_url, $replacements, $cdn_url ) {
		...
		$this->replacements = array_unique(
			array_map( [ $this, 'protocolize_url' ],
				apply_filters( 'cloud_uploads_replacement_urls', $replacements )
			)
		);
		...
		$filelist   = new Cloud_Uploads_Filelist( '/' ); //path doesn't matter
		$exclusions = apply_filters( 'cloud_uploads_sync_exclusions', $filelist->exclusions );
	...
	protected function rewrite_url( $matches ) {
		 ...
		 *
		 * @param  {string}  $replace  The url to replace the match with, like `https://xxxxx.clouduploads.cloud/somefile.png`.
		 ...
		 * @since  1.0
		 * @hook   cloud_uploads_rewrite_url
		 *
		 */
		return apply_filters( 'cloud_uploads_rewrite_url', $replace, $matches );
	}
}
